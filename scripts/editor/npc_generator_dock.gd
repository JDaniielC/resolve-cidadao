...
# This script creates a dock panel in the Godot editor for NPC generation
...
func _ready():
	...
	# Delay initialization to ensure editor is ready
	await get_tree().process_frame
	_setup_dock()

func _setup_dock():
	var editor_interface = get_script().resource_path
	var dock = NPCGeneratorPanel.new()
	get_tree().root.add_child(dock)

class NPCGeneratorPanel extends Control:
	...
	func _on_generate_pressed():
		...
		editor_interface.open_scene_from_path(data.path)
